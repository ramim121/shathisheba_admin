...
async function decisionEffects(type: string, id: unknown, approve: boolean, result: Row, note: string | null): Promise<string[]> {
  ...
  if (type === "listing") {
    if (approve) {
      out.push("Listing is active and offered to buyers");
      const product = result.product as Row | null | undefined;
      if (product?.sku) out.push(`Product ${product.sku} published in Buy from Shathi`);
      out.push(describeDelivery(await notifyListing(id, "listing_approved"), "Farmer"));
    } else {
      out.push("Listing marked rejected");
      out.push(describeDelivery(await notifyListing(id, "listing_rejected", reasonVars(note)), "Farmer"));
    }
  } else if (type === "enrollment") {
